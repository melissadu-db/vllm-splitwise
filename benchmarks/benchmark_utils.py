...
def json_encode_lifetime_events(events: List[LifetimeEvent]) -> List[dict]:
    return [{"timestamp": event.timestamp, "event_type": str(event.event_type)} for event in events]


def json_decode_lifetime_events(json_list: List[dict]) -> List[LifetimeEvent]:
    return [LifetimeEvent(LifetimeEventType(event["event_type"]), event["timestamp"]) for event in json_list]
...
class Dataset:
    ...
    def load(input_path: str):
        ...
        return Dataset(loaded_data["dataset_name"],
...
def read_request_results(path: str) -> List[RequestResult]:
    with open(path, "r") as f:
        request_results: List[RequestResult] = [
            RequestResult(
                item["prompt_len"], item["output_len"], item["start_time"], item["end_time"], item["token_timestamps"],
                json_decode_lifetime_events(item["lifecycle_events"])
                if item.get("lifecycle_events", None) is not None else None) for item in json.load(f)
        ]
...
BACKEND_TO_PORTS = {"vllm": 8100, "lightllm": 8200, "distserve": 8400}
